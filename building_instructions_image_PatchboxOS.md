...
- Boot the Rpi and folow the [first run tutorial](https://blokas.io/patchbox-os/docs/first-run-options/)
  - ssh to th MPU: `ssh patch@ip_address` (The default user name is *patch* and its password is *blokaslabs*)
    - update PatchBox
...
- Enter PiSound configuration: `sudo pisound-config`
  ...
  - Change Pisound HotSpot settings:
    - ssid: `MPU00X` (use MPU's ID)
    - wpa_passphrase: `mappings` (default MPU password)
...
- Enter Raspi-Config: `sudo raspi-config`
  ...
  - System options:
    - Hostname: `MPU00X` (use MPU's ID)

Create a place for the patches and docs: `mkdir ~/Patches ~/Documents ~/sources`

Reboot

- To access MODEP, use [http://mpu00X.local/](http://mpu00X.local/) (replace MPU number) or the MPU's IP address
...
```bash
scp <PATH_CONTAINING_FILES>/* patch@<MPU_IP>:~/Patches/
scp <PATH_CONTAINING_FILES>/* patch@<MPU_IP>:~/Documents/
```
...
Create Samba configuration file. **Don't forget to replace the MPU number!**

```bash
cat <<- "EOF" | sudo tee /etc/samba/smb.conf
[global]
        server string = MPU00X
...
# The wiring for the LCD is as follows (MPU with PiSound):
#
...
# Python code to update the LCD with MPU info
...
[Unit]
 Description=MPU status print on LCD
 After=multi-user.target
...
# Python code to send button info (OSC) (MPU)
# Author: Edu Meneses (IDMIL, 2020)

# The wiring for the LCD is as follows (MPU002):
#
...
parser = argparse.ArgumentParser(description='Python code to send button info using OSC (MPU).')
...
parser.add_argument('-n', '--namespace', dest='namespace', type=str, default="/guitarami_mpu",
                    help='Namespace for the OSC messages. Default: /guitarami_mpu')
...
- MPU00X
  ...
  - MPU Hostname: `sudo raspi-config`
...
To access webmapper in your browser, go to `http://mpu003.local:50000/`(change the MPU address accordingly)
...
- Enter Raspi-Config: `sudo raspi-config`
  - System options:
    - Hostname: `MPU00X` (use MPU's ID)
- `sudo tee /etc/samba/smb.conf`, change all "MPU" references
